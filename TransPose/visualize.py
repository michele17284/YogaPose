...
keypoint_name = {
	0: "nose", 1: "eye(l)", 2: "eye(r)", 3: "ear(l)", 4: "ear(r)", 5: "sho.(l)", 6: "sho.(r)", 7: "elb.(l)", 8: "elb.(r)", 9: "wri.(l)", 10: "wri.(r)", 11: "hip(l)", 12: "hip(r)", 13: "kne.(l)", 14: "kne.(r)", 15: "ank.(l)", 16: "ank.(r)", 17: "random", 18: "random", }


class plt_config:
	def __init__(self, dataset_name):
		assert dataset_name == "coco", "{} dataset is not supported".format(dataset_name)
		self.n_kpt = 17
		# edge , color
	...
	for i in range(n_kpt):
		...
		for j in range(len(skeletons)):
			if len(skeletons[j][i]) > 2 and skeletons[j][i, 2] > 0:
				cv2.circle(canvas, tuple(skeletons[j][i, 0:2].astype("int32")), 3, (255, 255, 255), thickness=-1, )
...
def inspect_atten_map_by_locations(
	...
	atten_maps_hooks = [model.global_encoder.layers[i].self_attn.register_forward_hook(lambda self, input, output: global_enc_atten_maps.append(output[1])) for i in range(len(model.global_encoder.layers))]
	...
	for id, attn_map in enumerate(enc_atten_maps_hwhw):
		for p_id, p in enumerate(attn_map_pos):
			...
			if threshold is not None:
				...
				im = axs[id][p_id + 1].imshow(attention_map_for_this_point, cmap="nipy_spectral", alpha=0.79)
			else:
				im = axs[id][p_id + 1].imshow(attention_map_for_this_point, cmap="nipy_spectral", alpha=0.79)
			...
			if id == attn_layers_num - 1:
				axs[id][p_id + 1].set_xlabel("{}".format(keypoint_name[p_id]), fontsize=25, )
